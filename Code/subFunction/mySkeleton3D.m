function skel = mySkeleton3D(img,spare)
...
while( unchangedBorders < 6 )  % loop until no change for all six border types
    ...
    if unchangedBorders>0
       
       fprintf(['The first iteration(some direction may not thinning):',num2str(iterNum),'\n', ]);
    end  
end
fprintf(['Final iteration number is:',num2str(iterNum),'\n', ]);
