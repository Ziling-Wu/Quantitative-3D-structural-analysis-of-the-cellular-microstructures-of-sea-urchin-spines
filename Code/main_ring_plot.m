...
PlotScatter(tempBran,8,[0, 0.9, 0.9],0)%[0.8500, 0.3250, 0.0980]

    hold on

    PlotScatter(tempRing(:),30,'r',0)
    hold on
%     
% PlotScatter(tempNode(:),50,'k',0)
   hold on
   PlotScatter(trimskel,1,[0.6,0.6,0.6],0)
FigureFormat
...
PlotScatter(tempBran,8,[0.8500, 0.3250, 0.0980],0)%[0.8500, 0.3250, 0.0980]
...
PlotScatter(tempBran,8,[0,0.5,0],0)%[0,0.5,0]
    hold on
...
PlotScatter(tempBran,8,'b',0)%'b'[0.5,0.5,0.5]
hold on 
PlotScatter(tempRing(:),30,'r',0)
% hold on
% PlotScatter(tempNode(:),50,'k',0)
...
PlotScatter(trimskel,1,[0.6,0.6,0.6],0)
FigureFormat
% saveas(gcf,'ringlessthan5_all','epsc');
% saveas(gcf,'ringlessthan5_all','pdf');
% saveas(gcf,'ringlessthan5_all','png');
%% crop one 4-E ring
load cont
clf
ring4 = ring{1};
...
PlotScatter(tempBran,8,[0.8500, 0.3250, 0.0980],0)%
    ...
    hold on
%     
PlotScatter(cont,1,[0.4,0.4,0.4],0)
   hold on
   PlotScatter(trimskel,2,[0.6,0.6,0.6],0)
FigureFormat
view(105,13)
...
% PlotScatter_datas(img,2,[0.6,0.6,0.6],0,size(img))
% view(105,13)
FigureFormat
for i =1:size(img,3)
    im = img(:,:,i);
...
PlotScatter(tempBran,8,[59,84,164]/255,0)%[0,0.5,0]
    hold on
...
% ring4
clf
mean(2*sqrt(allRingSize4/pi)*0.65)
std(2*sqrt(allRingSize4/pi)*0.65)
nbins = [28:4:60]*0.65;
histogram(2*sqrt(allRingSize4/pi)*0.65,nbins,'FaceColor',[0, 0.9, 0.9],'facealpha',.5)
ylim([0,350])
xlim([15,50])
yticks([0:50:400])
xticks([15:5:50])
...
% ring5
clf
mean(2*sqrt(allRingSize5/pi)*0.65)
std(2*sqrt(allRingSize5/pi)*0.65)
nbins = [28:4:60]*0.65;
histogram(2*sqrt(allRingSize5/pi)*0.65,nbins,'FaceColor',[0.8500, 0.3250, 0.0980],'facealpha',.5)
ylim([0,350])
xlim([15,50])
yticks([0:50:400])
xticks([15:5:50])
...
%% ring6
clf
mean(2*sqrt(allRingSize6/pi)*0.65)
std(2*sqrt(allRingSize6/pi)*0.65)
nbins = [30:4:70]*0.65;
histogram(2*sqrt(allRingSize6/pi)*0.65,nbins,'FaceColor',[0, 0.5, 0],'facealpha',.5)
ylim([0,350])
xlim([15,50])
yticks([0:50:400])
xticks([15:5:50])
...
%% ring7
clf
mean(2*sqrt(allRingSize7/pi)*0.65)
std(2*sqrt(allRingSize7/pi)*0.65)
nbins = [35:3:70]*0.65;
histogram(2*sqrt(allRingSize7/pi)*0.65,nbins,'FaceColor','b','facealpha',.5)
ylim([0,350])
xlim([15,50])
yticks([0:50:400])
xticks([15:5:50])
...
mean(2*sqrt(allRingSize/pi)*0.65)
std(2*sqrt(allRingSize/pi)*0.65)
clf
nbins = [25:3:80]*0.65;
histogram(2*sqrt(allRingSize/pi)*0.65,nbins,'FaceColor','b','facealpha',.5)
ylim([0,350])
xlim([15,50])
yticks([0:50:400])
xticks([15:5:50])
...
for i =1%size(lp6,1)
   ...
   for j=1:length(lp)-1
    ...
    PlotScatter(temp,30,'b')
    hold on

    PlotScatter(lp',100,'r')
    hold on
end
hold on
PlotScatter(node(np,1),200,'k')
hold on
tmpbran1 = node(np,1);
[x,y,z]=meshgrid(1:datas(1),1:datas(2),1:datas(3));
 [x0,y0,z0] = ind2sub(datas,tmpbran1);
 tempsphere = cont.*(((x-y0).^2+(y-x0).^2+(z-z0).^2)<=50^2);
%  PlotScatter(tempsphere,10,[1,1,1]-0.3)
%  hold on
% PlotScatter(trimskel,10,[1,1,1]-0.3,0)
% FigureFormat
...
% PlotScatter(tempBran,10,[0.8500, 0.3250, 0.0980],0)
...
PlotScatter(tempNode(:),150,[0.8500, 0.3250, 0.0980],0)
%    hold on
%    PlotScatter(trimskel,1,[0.6,0.6,0.6],0)
% FigureFormat
% saveas(gcf,'ring5lessthan8','epsc');
...
% PlotScatter(tempBran,10,[0, 0.5, 0],0)
%     hold on
% 
%     PlotScatter(tempRing(:),30,'r',0)
%     hold on
    
PlotScatter(tempNode(:),90,[0, 0.5, 0],0)
%    hold on
...
% PlotScatter(tempBran,10,'b',0)
% hold on 
% PlotScatter(tempRing(:),30,'r',0)
hold on
PlotScatter(tempNode(:),50,'b',0)
hold on
PlotScatter(trimskel,1,[0.6,0.6,0.6],0)
FigureFormat
